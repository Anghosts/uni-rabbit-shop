...
import {
  reqGetMemberOrderPre,
  reqGetMemberOrderPreNow,
  reqMemberOrder,
  reqGetMemberOrderRepurchase,
} from '@/services/order'
...
const getMemberOrderPre = async () => {
  ...
  if (skuId && count) {
    /* 立即购买 */
    const { result } = await reqGetMemberOrderPreNow({
      skuId,
      count,
      addressId,
    })
    ...
    /* 再次购买 */
    const { result } = await reqGetMemberOrderRepurchase(query.orderId)
    orderPre.value = result
  } else {
    /* 预付订单 */
    const { result } = await reqGetMemberOrderPre()
    orderPre.value = result
  }
}
...
const onOrderSubmit = async () => {
  if (!address.value?.id) {
    ...
    const { result } = await reqMemberOrder({
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !address?.id }" @tap="onOrderSubmit"> 提交订单 </view>
  </view>
</template>
