...
import {
  reqGetMemberOrderDetail,
  reqGetMemberOrderConsignment,
  reqMemberOrderReceipt,
  reqGetMemberOrderLogistics,
  reqDeleteMemberOrder,
  reqMemberOrderCancel,
} from '@/services/order'
import { reqGetPayWxPayMiniPay, reqGetPayMock } from '@/services/pay'
...
const getOrderDetail = async () => {
  const { result } = await reqGetMemberOrderDetail(query.id)
  order.value = result
  getLogistic()
}
...
const onOrderPay = async () => {
  if (isDev) {
    await reqGetPayMock(query.id)
  } else {
    const { result } = await reqGetPayWxPayMiniPay(query.id)
...
const onOrderSend = async () => {
  if (isDev) {
    await reqGetMemberOrderConsignment(query.id)
...
const onOrderConfirm = () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const { result } = await reqMemberOrderReceipt(query.id)
        order.value = result
        getLogistic()
      }
    },
...
// 取消订单
const onOrderCanel = async () => {
  if (reason.value) {
    const { result } = await reqMemberOrderCancel(query.id, reason.value)
    order.value = result
    popup.value.close()
...
const onOrderDelete = () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        await reqDeleteMemberOrder([query.id])
        uni.redirectTo({
...
const getLogistic = async () => {
  if (
    ...
    const { result } = await reqGetMemberOrderLogistics(query.id)
  ...
  <uni-popup ref="popup" type="bottom" background-color="#fff">
    <view class="popup-root">
      ...
      <view class="footer">
        ...
        <view class="button primary" @tap="onOrderCanel">确认</view>
      </view>
    </view>
  </uni-popup>
</template>
