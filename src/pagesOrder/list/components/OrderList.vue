...
import { ref } from 'vue'
import {
  reqGetMemberOrderList,
  reqMemberOrderReceipt,
  reqDeleteMemberOrder,
} from '@/services/order'
import { reqGetPayWxPayMiniPay } from '@/services/pay'
...
const getOrderList = async () => {
  if (pageStatus.value !== 'ok') return
  pageStatus.value = 'loading'

  const { result } = await reqGetMemberOrderList(queryParams)
  // 追加数据
  ...
  if (queryParams.page < result.pages) {
    ...
    // 已全部加载完成
    pageStatus.value = 'end'
  }
}

// 数据加载状态
const pageStatus = ref('ok')
// 下拉刷新状态
...
const onOrderPay = async (id: string) => {
  if (isDev) {
    await reqGetPayMock(id)
  } else {
    const { result } = await reqGetPayWxPayMiniPay(id)
...
const onOrderConfirm = (id: string) => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        await reqMemberOrderReceipt(id)
        const order = orderList.value.find((v) => v.id === id)
...
<template>
  ...
  >
    ...
    <!-- 底部提示文字 -->
    <view class="loading-text" :style="{ paddingBottom: safeAreaInsets?.bottom + 10 + 'px' }">
      {{ pageStatus === 'end' ? '没有更多数据~' : '正在加载...' }}
    </view>
  </scroll-view>
</template>
